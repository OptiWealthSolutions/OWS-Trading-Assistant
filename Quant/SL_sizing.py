...
import pandas as pd
import numpy as np
from risk_management import gestion_risque_adaptative, std, vol


# --------------- Fonction average true range index ----------------
def atr_index(ticker, window, duration):
...
entry_price_test = 1
sl = sl_sizing(atr,k,entry_price_test,"BUY")
...
def position_sizing(pair, entry_price, direction, capital, account_currency="EUR"):
    ...
    stop_price = sl_sizing(atr, k, entry_price, direction)
    result_risk = gestion_risque_adaptative(capital, std, vol)
    #result_risk = pd.DataFrame(result_risk)
    risk_amount = result_risk["Montant à risquer (€)"]
